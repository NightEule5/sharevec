...
impl<T, const ATOMIC: bool> Weak<T, Global, ATOMIC> {
	...
	///
	/// ```
	/// use sharevec::deque::rc::{Deque, Weak};
	///
	/// let strong = Deque::from(b"Hello!");
	///
...
impl<T, A: Allocator, const ATOMIC: bool> Weak<T, A, ATOMIC> {
	...
	/// use std::ptr;
	///
	/// let mut strong = Deque::from(b"Hello!");
	/// let weak = strong.demote();
	/// // Both point to the same data.
	/// let slice = strong.make_contiguous();
	/// assert!(ptr::eq(&*slice, weak.as_ptr()));
	...
	/// use std::alloc::System;
	///
	/// let mut strong = Deque::<u8, _>::new_in(System);
	...
	/// Converts a raw pointer previously returned by [`into_raw`] back into [`crate::array::deque::Weak`].
	///
	/// This can be used to safely get a strong reference (by calling [`promote`]) or to decrement
	/// the weak count by dropping the returned pointer.
	///
	...
	/// use sharevec::deque::rc::Deque;
	///
	/// let mut vec = Deque::from(b"Hello!");
	...
	/// ```
	pub fn promote(&self) -> Option<Deque<T, ATOMIC, A>> {
		todo!()
	}
	...
	/// use sharevec::deque::rc::{Deque, Weak};
	///
	/// let first_vec = Deque::from(b"Hello!");
	/// let first = first_vec.demote();
	/// let second = first_vec.demote();
	/// assert!(first.ptr_eq(&second));
	///
	/// let third = Deque::from(b"Hello!").demote();
	...
	/// assert!(first.ptr_eq(&second));
	///
	/// let third = Deque::<u32>::new().demote();
	/// assert!(!first.ptr_eq(&third));
	/// ```
	#[must_use]
