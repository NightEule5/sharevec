...
impl<T, const N: usize, A: Allocator, const ATOMIC: bool> IntoIter<T, N, A, ATOMIC> {
	pub(super) fn new_owned(vec: ArrayVec<T, N, ATOMIC, A>) -> Self {
		Self::new(Mode::Owned, vec)
	}

	pub(super) fn new_cloned(vec: ArrayVec<T, N, ATOMIC, A>) -> Self where T: Clone {
		Self::new(Mode::Cloned, vec)
	}

	#[allow(clippy::multiple_unsafe_ops_per_block)]
	fn new(mode: Mode, vec: ArrayVec<T, N, ATOMIC, A>) -> Self {
		unsafe {
			...
			Self { mode, buf, _t: PhantomData, alloc, ptr: buf, end }
		}
	}
...
impl<T, const N: usize, A: Allocator, const ATOMIC: bool> Drop for IntoIter<T, N, A, ATOMIC> {
	fn drop(&mut self) {
		...
		struct DropGuard<'a, T, const N: usize, A: Allocator, const ATOMIC: bool> {
			buf: NonNull<T>,
			alloc: &'a mut ManuallyDrop<A>,
		};
		
		impl<T, const N: usize, A: Allocator, const ATOMIC: bool> Drop for DropGuard<'_, T, N, A, ATOMIC> {
			fn drop(&mut self) {
				...
				//  a vector.
				let mut vec = unsafe {
					RawVec::<[T; N], A>::from_non_null(self.buf, alloc)
				};
		...
		let len = self.len();
		let Self { mode, buf, alloc, ptr, .. } = self;
		// Even if `drop_in_place` panics, the vector will be deallocated.
		let guard: DropGuard<T, N, A, ATOMIC> = DropGuard { buf: *buf, alloc };
