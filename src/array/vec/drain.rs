...
pub struct Drain<'a, T: 'a, const N: usize, A: Allocator + 'a, const ATOMIC: bool> {
	...
	tail: Range<usize>,
	iter: Iter<'a, T>,
	vec: NonNull<ArrayVec<T, N, ATOMIC, A>>,
}
...
impl<'a, T: 'a, const N: usize, A: Allocator + 'a, const ATOMIC: bool> Drain<'a, T, N, A, ATOMIC> {
	pub(super) fn unique(
		tail: Range<usize>,
		iter: Iter<'a, T>,
		vec: &'a mut ArrayVec<T, N, ATOMIC, A>
	) -> Self {
	...
	fn new(mode: Mode, tail: Range<usize>, iter: Iter<'a, T>, vec: &'a mut ArrayVec<T, N, ATOMIC, A>) -> Self {
		Self {
			mode,
			tail,
			iter,
...
fn drop_unique<T, const N: usize, A: Allocator>(
	...
	impl<T, const N: usize, A: Allocator> Drop for DropGuard<'_, T, N, A> {
		fn drop(&mut self) {
...
fn drop_clone<T, const N: usize, A: Allocator, const ATOMIC: bool>(
	...
	struct AllocGuard<'alloc, T, const N: usize, A: Allocator> {
		vec: RawVec<[T; N], &'alloc A>
	}
	
	impl<T, const N: usize, A: Allocator> Drop for AllocGuard<'_, T, N, A> {
		fn drop(&mut self) {
